    ...
    <div className="flex items-center gap-1 mt-1 mb-1">
      {data.map((reaction) => (
        ...
        >
          <button
            ...
            className={cn(
              'h-6 px-2 rounded-full bg-slate-200/70 border border-transparent test-slate-800 items-center flex gap-x-1 cursor-pointer',
              reaction.memberIds.includes(currentMemberId) &&
                'bg-blue-100/70 border-blue-500 text-white cursor-pointer'
            )}
          >
            {reaction.value}
            <span
      ...
      >
        <button className="h-7 px-3 rounded-full bg-slate-200/70 border border-transparent hover:border-slate-500 text-slate-800 flex items-center gap-x-1">
          <MdOutlineAddReaction className="size-4 cursor-pointer" />
        </button>
      </EmojiPopover>
    </div>
  )
